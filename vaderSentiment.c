...
int is_all_caps(const char *word) {
    ...
    return 1; // All alphabetic characters are uppercase and no forbidden symbols were found
}
...
float calculate_score(const char *sentence, WordData lexicon[], int lexicon_size) {
    ...
    printf("Exclamation marks found: %d, Total exclamation boost: %.4f, Final sentence score: %.4f\n", exclamation_count, exclamation_boost, total_score);
    ...
    return calculate_compound_score(total_score);
}